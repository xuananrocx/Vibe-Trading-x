...
### Added
- **中英文界面切换.** 完整的 i18n 支持，侧边栏语言切换按钮，支持简体中文和英文界面
- **AI 日志模块.** 新增 LLM 交互日志记录和查看功能，包含系统提示词、用户消息、AI 回复、Token 用量等完整信息
- **实时流式输出修复.** 修复 SSE 通过 Vite 代理的缓冲问题，AI 回复现在实时流式显示，不再卡在"智能体工作中"
- **API 网关伪装支持.** LLM 客户端支持伪装 Claude Code 请求头，可通过受限 API 网关调用 Anthropic 模型

### Changed
- **前端优化.** 修复 React 渲染冲突，优化日志展示交互，改进删除确认流程
- **后端日志增强.** 支持逐轮写入日志，前端定时轮询实现"进行中"状态显示

### Fixed
- **SSE 实时显示.** 修复 Vite 代理配置，解决事件流缓冲导致前端无法实时接收 AI 回复的问题
- **日志数据类型.** 修复 response_text 序列化问题，确保日志数据正确渲染不崩溃
- **用户交互体验.** 改进删除确认对话框，避免原生弹窗被页面容器截断
