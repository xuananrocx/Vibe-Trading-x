...
@app.get("/llm-logs", dependencies=[Depends(require_auth)])
async def list_llm_logs(
    ...
    return get_llm_log_store().list_logs(session_id=session_id, limit=limit, offset=offset)


@app.get("/llm-logs/{log_id}", dependencies=[Depends(require_auth)])
async def get_llm_log(log_id: str):
    from src.core.llm_log import get_llm_log_store
    log = get_llm_log_store().get_log(log_id)
    if not log:
        raise HTTPException(status_code=404, detail="LLM log not found")
    return log


@app.delete("/llm-logs/{log_id}", dependencies=[Depends(require_auth)])
async def delete_llm_log(log_id: str):
    from src.core.llm_log import get_llm_log_store
    deleted = get_llm_log_store().delete_log(log_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="LLM log not found")
    return {"status": "deleted"}


@app.delete("/llm-logs", dependencies=[Depends(require_auth)])
async def clear_llm_logs(session_id: Optional[str] = Query(None)):
    from src.core.llm_log import get_llm_log_store
    count = get_llm_log_store().clear_logs(session_id=session_id)
