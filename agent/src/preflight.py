...
def _check_llm_provider() -> CheckResult:
    ...
    # Anthropic provider uses its own env vars
    if provider.lower() == "anthropic":
        ...
        display_url = base_url or "https://api.anthropic.com"
        try:
            import requests
            ping_url = base_url.rstrip("/") if base_url else "https://api.anthropic.com"
            requests.get(ping_url, timeout=10)
            return CheckResult(
                name=f"LLM ({provider})",
                status="ready",
                message=f"{model} via {display_url}",
                impact="",
            )
        except Exception as exc:
            return CheckResult(
