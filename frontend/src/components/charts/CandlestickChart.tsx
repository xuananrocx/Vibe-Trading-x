...
export function CandlestickChart({ data, markers, indicators, height = 500 }: Props) {
  ...
  useEffect(() => {
    ...
    if (overlays.has("boll")) {
      ...
      overlaySeries.push(
        { name: "BOLL+", type: "line", data: boll.upper, xAxisIndex: 0, yAxisIndex: 0, symbol: "none", lineStyle: { color: th.bollColor, width: 0.8, type: "dashed" } },
        { name: "BOLL", type: "line", data: boll.mid, xAxisIndex: 0, yAxisIndex: 0, symbol: "none", lineStyle: { color: th.bollColor, width: 1 } },
        { name: "BOLL-", type: "line", data: boll.lower, xAxisIndex: 0, yAxisIndex: 0, symbol: "none", lineStyle: { color: th.bollColor, width: 0.8, type: "dashed" } },
      );
    ...
    const marks = (markers || []).map(m => ({
      ...
      itemStyle: { color: m.side === "BUY" ? th.upColor : th.downColor },
    ...
    const vol = data.map((d, i) => ({
      value: d.volume,
      itemStyle: { color: closes[i] >= opens[i] ? th.volumeUp : th.volumeDown },
    }));
    ...
    let subSeries: any[] = [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let subYAxis: any = { scale: true, gridIndex: 1, splitLine: { lineStyle: { color: th.gridColor } }, axisLabel: { color: th.textColor, fontSize: 10 } };
    ...
    if (sub === "vol") {
      ...
      subSeries = [
        { name: "DIF", type: "line", data: m.dif, xAxisIndex: 1, yAxisIndex: 1, symbol: "none", lineStyle: { width: 1, color: th.infoColor } },
        { name: "DEA", type: "line", data: m.signal, xAxisIndex: 1, yAxisIndex: 1, symbol: "none", lineStyle: { width: 1, color: th.warningColor } },
        { name: "MACD", type: "bar", data: m.histogram.map(v => ({ value: v ?? 0, itemStyle: { color: (v ?? 0) >= 0 ? th.upColor : th.downColor } })), xAxisIndex: 1, yAxisIndex: 1 },
      ];
      legendNames.push("DIF", "DEA", "MACD");
    } else if (sub === "rsi") {
      subSeries = [{ name: "RSI", type: "line", data: indicatorCache.rsi, xAxisIndex: 1, yAxisIndex: 1, symbol: "none", lineStyle: { width: 1.5, color: th.infoColor } }];
      subYAxis = { ...subYAxis, min: 0, max: 100 };
      legendNames.push("RSI");
    } else {
      const kdj = indicatorCache.kdj;
      subSeries = [
        { name: "%K", type: "line", data: kdj.k, xAxisIndex: 1, yAxisIndex: 1, symbol: "none", lineStyle: { width: 1, color: th.infoColor } },
        { name: "%D", type: "line", data: kdj.d, xAxisIndex: 1, yAxisIndex: 1, symbol: "none", lineStyle: { width: 1, color: th.warningColor } },
    ...
    chart.setOption({
        ...
        formatter: (params: any) => {
          ...
          for (const p of params) {
            if (p.seriesName === "K" && Array.isArray(p.value)) {
              ...
              const clr = chg >= 0 ? th.upColor : th.downColor;
              html += `<br/>${t("charts.open")} ${open.toFixed(2)}&nbsp; ${t("charts.high")} ${high.toFixed(2)}`;
              html += `<br/>${t("charts.low")} ${low.toFixed(2)}&nbsp; ${t("charts.close")}: <span style="color:${clr}"><b>${close.toFixed(2)}</b> ${chg >= 0 ? "+" : ""}${chg.toFixed(2)} (${chg >= 0 ? "+" : ""}${pct}%)</span>`;
            } else if (p.seriesName === "Vol") {
              html += `<br/>${t("charts.vol")} ${abbreviateNum(Number(p.value))}`;
      ...
      toolbox: {
        feature: { saveAsImage: { title: t("charts.save") }, dataZoom: { title: { zoom: t("charts.zoom"), back: t("charts.reset") } }, restore: { title: t("charts.reset") } },
        right: 8, top: 0, iconStyle: { borderColor: th.textColor },
      },
      legend: { data: legendNames, textStyle: { color: th.textColor, fontSize: 10 }, right: 80, top: 2, type: "scroll", itemWidth: 12, itemHeight: 8, itemGap: 8 },
      grid: [
        { left: 8, right: 8, top: 36, height: "55%", containLabel: true },
        { left: 8, right: 8, top: "66%", height: "22%", containLabel: true },
      ],
      xAxis: [
        { type: "category", data: dates, gridIndex: 0, axisLine: { lineStyle: { color: th.axisColor } }, axisLabel: { color: th.textColor, fontSize: 10 }, boundaryGap: true },
        { type: "category", data: dates, gridIndex: 1, axisLine: { lineStyle: { color: th.axisColor } }, axisLabel: { show: false }, boundaryGap: true },
      ],
      yAxis: [
        { scale: true, gridIndex: 0, splitLine: { lineStyle: { color: th.gridColor } }, axisLabel: { color: th.textColor, fontSize: 10 } },
        subYAxis,
      ],
      dataZoom: [
  ...
  if (data.length === 0) {
    return <div className="text-muted-foreground text-sm p-4">{t("charts.noPriceData")}</div>;
  }
    ...
    <div>
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        ...
        <div className="relative">
          ...
          >
            {t("charts.indicators", { count: overlays.size })} <ChevronDown className="h-3 w-3" />
          </button>
          {showMenu && (
