...
function formatLeverage(leverage: MandateProfile["leverage"], t: ReturnType<typeof useTranslation>["t"]): string {
  if (typeof leverage === "number") {
    return leverage <= 1 ? t("chat.noLeverage") : t("chat.leverage", { value: leverage });
  }
  const lowered = leverage.toLowerCase();
  return lowered === "none" || lowered === "" ? t("chat.noLeverage") : leverage;
}
    ...
    >
      ...
      <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px]">
        <div className="col-span-2">
          <dt className="text-muted-foreground">{t("chat.universe")}</dt>
          <dd className="font-medium text-foreground">{formatUniverse(profile.universe)}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.maxOrder")}</dt>
          <dd className="font-mono font-medium text-foreground">{formatUsd(profile.max_order_usd)}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.dailyCap")}</dt>
          <dd className="font-mono font-medium text-foreground">{t("chat.tradesPerDay", { count: profile.daily_trade_cap })}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.leverageLeverage")}</dt>
          <dd className="font-medium text-foreground">{formatLeverage(profile.leverage, t)}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.instruments")}</dt>
      ...
      {adjusting ? (
        <div className="mt-3 grid gap-2">
          <input
            ...
            placeholder={t("chat.adjustPlaceholder")}
        ...
        >
          ...
          {busy ? t("chat.committing") : t("chat.commit", { label: profile.label })}
        </button>
      )}
    </div>
  );
...
export const MandateProposalCard = memo(function MandateProposalCard({ proposal, committed, onAdjust }: Props) {
  ...
  const handleCommit = useCallback(
    async (ordinal: number) => {
      ...
      if (!broker) {
        toast.error(t("chat.commitFailed"));
        return;
      }
      ...
      try {
        ...
        setBusyOrdinal(null);
        toast.error(error instanceof Error ? error.message : t("chat.commitError"));
      }
    },
        ...
        <div className="flex-1 min-w-0">
          <span className="inline-flex max-w-full flex-wrap items-center gap-1.5 rounded-lg bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-400">
            ...
            <span className="shrink-0">
              {t("chat.mandateActive", { id: committed.selected_ordinal })}
            </span>
            {maxOrder != null && (
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {isReauth ? t("chat.mandateReauthorize") : t("chat.mandateRuntime")}
            </p>
