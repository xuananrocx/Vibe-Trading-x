...
function formatRelative(value: string | number | null | undefined, t: ReturnType<typeof useTranslation>["t"]): string {
  if (value == null || value === "") return t("chat.timeNever");
  const then = typeof value === "number"
    ? (value < 1_000_000_000_000 ? value * 1000 : value)
    : new Date(value).getTime();
  if (!Number.isFinite(then)) return t("chat.timeUnknown");
  const deltaSec = Math.round((Date.now() - then) / 1000);
  if (deltaSec < 0) return t("chat.timeJustNow");
  if (deltaSec < 60) return t("chat.timeSecAgo", { count: deltaSec });
  if (deltaSec < 3600) return t("chat.timeMinAgo", { count: Math.floor(deltaSec / 60) });
  if (deltaSec < 86_400) return t("chat.timeHourAgo", { count: Math.floor(deltaSec / 3600) });
  return t("chat.timeDayAgo", { count: Math.floor(deltaSec / 86_400) });
}

function formatCountdown(iso: string | undefined, t: ReturnType<typeof useTranslation>["t"]): { label: string; expired: boolean; soon: boolean } {
  if (!iso) return { label: "—", expired: false, soon: false };
  const target = new Date(iso).getTime();
  if (!Number.isFinite(target)) return { label: t("chat.timeUnknown"), expired: false, soon: false };
  const deltaSec = Math.round((target - Date.now()) / 1000);
  if (deltaSec <= 0) return { label: t("chat.expired"), expired: true, soon: false };
...
function summarizeLimits(limits: LiveMandateLimits | undefined, t: ReturnType<typeof useTranslation>["t"]): string {
  if (!limits) return "";
  const parts: string[] = [];
  ...
  if (limits.max_leverage != null) parts.push(limits.max_leverage <= 1 ? t("chat.noLeverage") : t("chat.leverage", { value: limits.max_leverage }));
  return parts.join(" · ");
}

function fallbackAuthorizeInstruction(t: ReturnType<typeof useTranslation>["t"]): string {
  return t("chat.fallbackAuthorize");
}
...
function BrokerRow({
  ...
  const authorizeInstruction = authorizeHint?.instruction
    ?? (authorizeFailed ? fallbackAuthorizeInstruction(t) : t("chat.loadingConnector"));
  const authorizeNote = t("chat.connectorHint");
  ...
  const toggleRunner = useCallback(async () => {
    ...
    try {
      if (runnerAlive) {
        ...
        toast.success(t("chat.runnerStopped", { broker: brokerKey }));
      } else {
        await api.startLiveRunner(brokerKey);
        toast.success(t("chat.runnerStarted", { broker: brokerKey }));
      }
      onRefresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("chat.runnerControlFailed"));
    } finally {
      setBusy(false);
    }
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-md border bg-background/60 p-2">
              ...
              <div className={["mt-0.5 text-xs font-semibold", runnerAlive ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"].join(" ")}>
                {runnerAlive ? t("chat.running") : t("chat.stopped")}
              </div>
            </div>
            <div className="rounded-md border bg-background/60 p-2">
              <div className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {mandate.expires_at && (
                  ...
                  >
                    ...
                    {countdown.expired ? t("chat.expired") : t("chat.expiresIn", { countdown: countdown.label })}
                  </span>
                )}
              </div>
              <div className="mt-0.5 font-mono text-[11px] text-foreground">
                {summarizeLimits(mandate.limits, t) || t("chat.limitsUnavailable")}
              </div>
            </div>
          ) : (
          ...
          <div className="flex items-center justify-between gap-2">
            {halted ? (
              ...
              <span className="text-[10px] text-muted-foreground">
                {runnerAlive ? t("chat.runtimeActive") : t("chat.idle")}
              </span>
            )}
            <button
              type="button"
              ...
              title={runnerAlive ? t("chat.stopRunnerTitle") : t("chat.startRunnerTitle")}
            >
              {busy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Power className="h-3 w-3" />}
              {runnerAlive ? t("chat.stopRunner") : t("chat.startRunner")}
            </button>
          </div>
        </>
      )}
  ...
  return (
    <div className="grid gap-2">
      <button
        ...
        aria-label={t("chat.connectorStatus")}
        aria-expanded={open}
      >
        <Activity className="h-3 w-3 shrink-0" />
        <span className="shrink-0">{t("chat.connectorRuntime")}</span>
        <span className="truncate text-muted-foreground">
          {authorizedCount > 0 ? t("chat.connected", { count: authorizedCount }) : t("chat.noConnector")}
        </span>
        {anyRunning && !isHalted && (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600 dark:text-emerald-400">
            <CircleDot className="h-2.5 w-2.5" />
            {t("chat.runningStatus")}
          </span>
        )}
        {isHalted && (
