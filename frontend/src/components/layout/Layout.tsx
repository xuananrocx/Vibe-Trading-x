...
import { BarChart3, Bot, Moon, Sun, Plus, Trash2, Pencil, MessageSquare, ChevronsLeft, ChevronsRight, Settings, Layers, Loader2, Globe, FileText } from "lucide-react";
...
export function Layout() {
  ...
  const NAV = [
    { to: "/", icon: BarChart3, label: t("nav.home") },
    { to: "/agent", icon: Bot, label: t("nav.agent") },
    { to: "/alpha-zoo", icon: Layers, label: t("nav.alphaZoo") },
    { to: "/settings", icon: Settings, label: t("nav.settings") },
    { to: "/ai-log", icon: FileText, label: t("nav.aiLog") },
    { to: "/correlation", icon: BarChart3, label: t("nav.correlation") },
  ];
    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className={cn(
        ...
        <div className={cn("border-t", collapsed ? "p-1 flex flex-col items-center gap-1" : "p-3 space-y-2")}>
          {collapsed ? (
            <>
              <button onClick={toggleLang} className="p-1.5 text-muted-foreground hover:text-foreground rounded transition-colors" title={i18n.language === "zh-CN" ? "English" : "中文"}>
