...
export const api = {
  ...
  sseUrl: (sid: string, options?: { replay?: "active" }) => {
    ...
    const sseBase = "http://localhost:8899";
    let url = withAuthQuery(`${sseBase}/sessions/${sid}/events`);
  ...
  swarmSseUrl: (id: string) => withAuthQuery(`http://localhost:8899/swarm/runs/${id}/events`),
  ...
  listLLMLogs: (params?: { session_id?: string; limit?: number; offset?: number }) => {
    const q = new URLSearchParams();
    if (params?.session_id) q.set("session_id", params.session_id);
    if (params?.limit) q.set("limit", String(params.limit));
    if (params?.offset) q.set("offset", String(params.offset));
    const qs = q.toString();
    return request<LLMLogItem[]>(`/llm-logs${qs ? `?${qs}` : ""}`);
  },
  getLLMLog: (logId: string) =>
    request<LLMLogDetail>(`/llm-logs/${encodeURIComponent(logId)}`),
  deleteLLMLog: (logId: string) =>
    request<{ status: string }>(`/llm-logs/${encodeURIComponent(logId)}`, { method: "DELETE" }),
  clearLLMLogs: (sessionId?: string) => {
    const q = sessionId ? `?session_id=${encodeURIComponent(sessionId)}` : "";
    return request<{ status: string; count: number }>(`/llm-logs${q}`, { method: "DELETE" });
  },
};
...
export interface LLMLogRound {
  round_number: number;
  messages: Record<string, unknown>[];
  response_text: string | Record<string, unknown>[] | null;
