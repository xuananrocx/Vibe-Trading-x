...
export function Agent() {
  ...
  const runPrompt = async (prompt: string) => {
    ...
    try {
      ...
      if (!sid) {
        ...
        console.log("[Agent] created new session:", sid);
      } else {
        console.log("[Agent] reusing session:", sid, "sseRef:", sseSessionRef.current);
      }
      setupSSE(sid);
      console.log("[Agent] SSE setup done, sending message...");
      await api.sendMessage(sid, finalPrompt);
      console.log("[Agent] message sent, waiting for SSE events...");
    } catch {
      act().setStatus("error");
  ...
  const handleHaltLive = useCallback(async () => {
    ...
    try {
      ...
      setLiveStatusRefresh((n) => n + 1);
      toast.success(t("agent.connectorRuntimeHalted"));
    } catch (error) {
  ...
  const handleExport = () => {
    ...
    const lines: string[] = [`# ${t("agent.chatExport")}`, ``, `Export time: ${new Date().toLocaleString()}`, ``];
    for (const msg of messages) {
      const time = new Date(msg.timestamp).toLocaleString();
      if (msg.type === "user") {
        lines.push(`## ${t("agent.user")} (${time})`, ``, msg.content, ``);
      } else if (msg.type === "answer") {
        lines.push(`## ${t("agent.assistant")} (${time})`, ``, msg.content, ``);
      } else if (msg.type === "error") {
        lines.push(`## ${t("agent.error")} (${time})`, ``, msg.content, ``);
      } else if (msg.type === "tool_call") {
        lines.push(`> ${t("agent.toolCall")}: ${msg.tool || "unknown"}`, ``);
      } else if (msg.type === "run_complete") {
        lines.push(`> ${t("agent.backtestComplete")}: ${msg.runId || ""}`, ``);
      }
    }
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (blockedExts.some((ext) => lowered.endsWith(ext))) {
      toast.error(t("agent.execNotAllowed"));
      return;
    }
  ...
  const timelineRows = useMemo<TimelineRow[]>(() => {
    const rows: TimelineRow[] = groups.map((g, i) => {
      ...
      const key = g.kind === "timeline" ? `tl_${g.msgs.map(m => m.id).join("_")}` : `msg_${g.msg.id}_${g.msg.timestamp}`;
              ...
              >
                ...
                {goalProgress.evidenceTotal > 0 && (
                  <span className="shrink-0 rounded bg-background px-1 font-mono text-[10px] text-primary" title="Evidence collected toward this research goal">
                    {t("agent.evidence", { count: goalProgress.evidenceTotal })}
                  </span>
                )}
                <ChevronDown
                  className={[
