...
export function Compare() {
  ...
  const METRICS: MetricDef[] = [
    { key: "total_return",           label: t("metrics.total_return"),         type: "pct", higherIsBetter: true },
    { key: "annualized_return",      label: t("metrics.annualized_return"),    type: "pct", higherIsBetter: true },
    { key: "sharpe",                 label: t("metrics.sharpe"),               type: "num", higherIsBetter: true },
    { key: "calmar_ratio",           label: t("metrics.calmar_ratio"),         type: "num", higherIsBetter: true },
    { key: "sortino_ratio",          label: t("metrics.sortino_ratio"),        type: "num", higherIsBetter: true },
    { key: "max_drawdown",           label: t("metrics.max_drawdown"),         type: "pct", higherIsBetter: false },
    { key: "volatility",             label: t("metrics.volatility"),           type: "pct", higherIsBetter: false },
    { key: "win_rate",               label: t("metrics.win_rate"),             type: "pct", higherIsBetter: true },
    { key: "profit_factor",          label: t("metrics.profit_factor"),        type: "num", higherIsBetter: true },
    { key: "avg_win",                label: t("metrics.avg_win"),              type: "pct", higherIsBetter: true },
    { key: "avg_loss",               label: t("metrics.avg_loss"),             type: "pct", higherIsBetter: false },
    { key: "trade_count",            label: t("metrics.trades"),               type: "int", higherIsBetter: true },
    { key: "max_consecutive_losses", label: t("metrics.max_consec_losses"),    type: "int", higherIsBetter: false },
    { key: "exposure_time",          label: t("metrics.exposure_time"),        type: "pct", higherIsBetter: true },
    { key: "avg_holding_period",     label: t("metrics.avg_holding_period"),   type: "days", higherIsBetter: false },
  ];
  ...
  return (
    <div className="p-8 max-w-4xl space-y-6">
      ...
      <div className="flex gap-4 items-end">
        ...
        <div className="flex-1">
          <label className="text-xs text-muted-foreground block mb-1">{t("compare.compareLabel")}</label>
      ...
      {(leftCurve.length > 0 || rightCurve.length > 0) && (
        <div className="border rounded-xl p-4">
          ...
          <EquityChartOverlay
            ...
            rightLabel={rightRun ? truncatePrompt(rightRun.prompt, 20) || t("compare.compareLabel") : t("compare.compareLabel")}
          />
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/40">
                <th className="text-left px-4 py-2.5 text-muted-foreground font-medium">{t("compare.tableMetric")}</th>
                <th className="text-right px-4 py-2.5 text-muted-foreground font-medium">{t("compare.tableBaseline")}</th>
                <th className="text-right px-4 py-2.5 text-muted-foreground font-medium">{t("compare.tableCompare")}</th>
                <th className="text-right px-4 py-2.5 text-muted-foreground font-medium">{t("compare.tableDelta")}</th>
              </tr>
            </thead>
            <tbody>
      ...
      {!hasData && !loading && (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm">{t("compare.hint")}</p>
        </div>
      )}
    </div>
  );
