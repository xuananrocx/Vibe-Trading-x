...
export function RunDetail() {
  ...
  const TABS: { id: Tab; label: string; icon: typeof BarChart3; hidden?: boolean }[] = [
    { id: "chart", label: t("runDetail.tabs.chart"), icon: BarChart3 },
    { id: "trades", label: t("runDetail.tabs.trades"), icon: List },
    { id: "validation", label: t("runDetail.tabs.validation"), icon: ShieldCheck, hidden: !hasValidation },
    { id: "runCard", label: t("runDetail.tabs.runCard"), icon: FileCheck2, hidden: !hasRunCard },
    { id: "code", label: t("runDetail.tabs.code"), icon: Code2 },
  ];
  ...
  if (!run) return (
    <div className="p-8 space-y-2">
      <p className="text-red-500 font-medium">{t("runDetail.notFound")}</p>
      <p className="text-sm text-muted-foreground">
        {t("runDetail.notFoundHint")}
      </p>
      <button
        onClick={() => navigate(-1)}
        className="text-sm text-primary hover:underline inline-flex items-center gap-1.5"
      >
        <ArrowLeft className="h-3.5 w-3.5" /> {t("common.goBack")}
      </button>
    </div>
  );
          ...
          <div className="ml-auto flex gap-1">
            {run.trade_log && run.trade_log.length > 0 && (
              <button
                ...
                title={t("runDetail.downloadTrades")}
              >
                <Download className="h-3.5 w-3.5" /> {t("runDetail.downloadTrades")}
              </button>
            )}
            {run.metrics && (
              <button
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="grid gap-3 md:grid-cols-4">
        <RunCardStat label={t("runDetail.card.schema")} value={card.schema_version || "unknown"} />
        <RunCardStat label={t("runDetail.card.generated")} value={formatRunCardValue(card.generated_at)} />
        <RunCardStat label={t("runDetail.card.dataSources")} value={dataSources.length ? dataSources.join(", ") : t("runDetail.card.noDataSources")} />
        <RunCardStat label={t("runDetail.card.warnings")} value={String(warnings.length)} tone={warnings.length ? "warning" : "normal"} />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <RunCardPanel title={t("runDetail.reproducibility")} icon={Fingerprint}>
          <KeyValueTable data={reproducibility} empty={t("runDetail.noReproducibility")} monospaceValues />
        </RunCardPanel>
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <RunCardPanel title={t("runDetail.metrics")} icon={BarChart3}>
          <KeyValueTable data={metrics} empty={t("runDetail.noMetrics")} />
        </RunCardPanel>
        <RunCardPanel title={t("runDetail.validation")} icon={ShieldCheck}>
          {card.validation ? (
            ...
            <p className="text-sm text-muted-foreground">{t("runDetail.noValidation")}</p>
          )}
        </RunCardPanel>
      </div>
      ...
      <RunCardPanel title={t("runDetail.artifactChecksums")} icon={FileCheck2}>
        {artifacts.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-left text-muted-foreground">
                  <th className="py-2 pr-4">{t("runDetail.tablePath")}</th>
                  <th className="py-2 pr-4">{t("runDetail.tableSize")}</th>
                  <th className="py-2">{t("runDetail.tableSha256")}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <p className="text-sm text-muted-foreground">{t("runDetail.noChecksums")}</p>
        )}
      </RunCardPanel>
    </div>
  );
  ...
  if (entries.length === 0 && !hasEquity) {
    return (
      <div className="p-8 text-center text-muted-foreground space-y-2">
        ...
        <p className="text-xs">{t("runDetail.noChartHint")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b text-left text-muted-foreground">
            <th className="py-2 pr-4">{t("runDetail.tableTime")}</th>
            <th className="py-2 pr-4">{t("runDetail.tableCode")}</th>
            <th className="py-2 pr-4">{t("runDetail.tableSide")}</th>
            <th className="py-2 pr-4">{t("runDetail.tablePrice")}</th>
            <th className="py-2 pr-4">{t("runDetail.tableQty")}</th>
            <th className="py-2">{t("runDetail.tableReason")}</th>
          </tr>
        </thead>
        <tbody>
          {trades.map((tr, i) => (
