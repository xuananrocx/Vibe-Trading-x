...
export function Settings() {
    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(t("settings.loadFailed", { error: message }));
          toast.error(t("settings.dataSourceLoadFailed", { error: message }));
        }
      })
      .finally(() => {
  ...
  const submitLocalApiKey = (event: FormEvent) => {
    ...
    toast.success(t("settings.localKeySaved"));
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(t("settings.llmSaveFailed", { error: error instanceof Error ? error.message : "Unknown error" }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.success(t("settings.dataSourceSaved"));
    } catch (error) {
      toast.error(t("settings.dataSourceSaveFailed", { error: error instanceof Error ? error.message : "Unknown error" }));
    } finally {
      setDataSaving(false);
    }
  };
  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{t("settings.localApi.description")}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          <span className={labelClass}>{t("settings.localApi.serverApiKey")}</span>
          <input
            type="password"
            value={localApiKey}
        ...
        >
          ...
          {t("settings.localApi.saveLocalKey")}
        </button>
      </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ? t("settings.llm.keyKeepCurrent")
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? t("settings.llm.oauthHint", { command: selectedProvider.login_command })
        : t("settings.llm.noKeyRequired");
  ...
  const tushareStatus = dataSettings.tushare_token_configured
    ? t("settings.dataSource.configured")
    : t("settings.dataSource.tokenKeepCurrent");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{t("settings.llm.description")}</p>
      </div>
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("settings.llm.connection")}</h2>
          </div>

          <div className="grid gap-4">
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.provider")}</span>
              <select
                value={form.provider}
              ...
              <span className={hintClass}>{t("settings.llm.providerHint")}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.model")}</span>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  title={t("settings.llm.useDefaults")}
                >
                  <RotateCcw className="h-4 w-4" />
                  <span className="hidden sm:inline">{t("settings.llm.useDefaults")}</span>
                </button>
              </div>
              <span className={hintClass}>{t("settings.llm.modelHint")}</span>
            </label>

            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.baseUrl")}</span>
              <input
                value={form.base_url}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>
                {selectedProvider?.auth_type === "oauth" ? t("settings.llm.oauth") : t("settings.llm.apiKey")}
              </span>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("settings.llm.generation")}</h2>
          </div>

          <div className="grid gap-4">
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.temperature")}</span>
              <input
                type="number"
                min={0}
                max={2}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.timeoutSeconds")}</span>
              <input
                type="number"
                min={1}
                max={3600}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.maxRetries")}</span>
              <input
                type="number"
                min={0}
                max={20}
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.llm.reasoningEffort")}</span>
              <select
                value={form.reasoning_effort}
                onChange={(event) => setForm({ ...form, reasoning_effort: event.target.value })}
                className={fieldClass}
              >
                <option value="">{t("settings.llm.off")}</option>
                <option value="low">{t("settings.llm.reasoningLow")}</option>
                <option value="medium">{t("settings.llm.reasoningMedium")}</option>
                <option value="high">{t("settings.llm.reasoningHigh")}</option>
                <option value="max">{t("settings.llm.reasoningMax")}</option>
              </select>
              <span className={hintClass}>{t("settings.llm.reasoningHint")}</span>
            </label>

            <div className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{t("settings.llm.savedTo")}: </span>
            ...
            >
              ...
              {saving ? t("settings.llm.saving") : t("settings.llm.saveSettings")}
            </button>
          </div>
        </section>
      </form>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("settings.dataSource.title")}</h2>
          </div>
          <p className="text-sm text-muted-foreground">{t("settings.dataSource.description")}</p>
        </div>

        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.dataSource.tushareToken")}</span>
              <div className="relative">
                <KeyRound className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
                <input
                  type="password"
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{t("settings.dataSource.tushareHint")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
                    ...
                    className="h-3.5 w-3.5 accent-primary"
                  />
                  {t("settings.dataSource.clearTushareToken")}
                </label>
              </div>
            </label>
            ...
            >
              ...
              {dataSaving ? t("settings.dataSource.saving") : t("settings.dataSource.saveSettings")}
            </button>
          </div>

          <div className="rounded-md border bg-muted/20 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <span className="text-sm font-medium">{t("settings.dataSource.baostock")}</span>
              <span className={`rounded-full px-2 py-0.5 text-xs ${dataSettings.baostock_supported ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                {dataSettings.baostock_supported ? t("settings.dataSource.loaderAvailable") : t("settings.dataSource.noProjectLoader")}
              </span>
            </div>
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p>
                {dataSettings.baostock_installed
                  ? t("settings.dataSource.packageInstalled")
                  : t("settings.dataSource.packageNotInstalled")}
              </p>
            </div>
          </div>
        </div>
